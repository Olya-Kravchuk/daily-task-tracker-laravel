                                        ...
                                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5" data-recurring-task-item>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                                                    {{ ucfirst($recurringTask['frequency']) }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                ...
                                                @elseif($recurringTask['frequency'] === 'monthly' && isset($recurringTask['frequency_config']['day_of_month']))
                                                    {{ __('Day') }} {{ $recurringTask['frequency_config']['day_of_month'] }}
                                                @else
                                                    -
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                @if($recurringTask['start_date'] || $recurringTask['end_date'])
                                                    ...
                                                    {{ optional($recurringTask['start_date'])->display ?? '...' }}
                                                    -
                                                    {{ optional($recurringTask['end_date'])->display ?? '...' }}
                                                @else
                                                    -
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <a href="{{ route('recurring-tasks.edit', ['recurring_task' => $recurringTask['id']]) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 mr-3">
                                                    {{ __('Edit') }}
                                                </a>
                                                <button type="button" data-recurring-task-delete data-id="{{ $recurringTask['id'] }}" class="cursor-pointer text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 disabled:opacity-50">
                                                    {{ __('Delete') }}
                                                </button>
                                            </td>
                                        </tr>
